...
#define SSD1306_I2C_ADDR 0x3C    // I2C address
...
#define SSD1306_SCREEN_BUFFER_SIZE                                             \
  (SSD1306_SCREEN_WIDTH * SSD1306_SCREEN_HEIGHT /                              \
   SSD1306_NUMBER_PAGES) // refer to the description below
#define SSD1306_WORK_BUFFER_SIZE (SSD1306_SCREEN_BUFFER_SIZE)
...
#define SSD1306_RIGHT_HORIZONTAL_SCROLL                                        \
  0x26 // initiate setup for right horizontal scroll
#define SSD1306_LEFT_HORIZONTAL_SCROLL                                         \
  0x27 // initiate setup for left horizontal scroll
#define SSD1306_VERTICAL_RIGHT_HORIZONTAL_SCROLL                               \
  0x29 // initiate setup for vertical and right horizontal scroll
#define SSD1306_VERTICAL_LEFT_HORIZONTAL_SCROLL                                \
  0x2A // initiate setup for vertical and left horizontal scroll
#define SSD1306_DEACTIVATE_SCROLL 0x2E
#define SSD1306_ACTIVATE_SCROLL 0x2F
#define SSD1306_VERTICAL_SCROLL_AREA                                           \
  0xA3 // initialize setup for vertical scroll area

// 3. Addressing Setting Command Table
#define SSD1306_SET_LOWER_COL_ADDR                                             \
  0x00 // should be ORed with value in range 0x0 - 0xF (refer datasheet, p. 34)
#define SSD1306_SET_HIGHER_COL_ADDR                                            \
  0x10 // should be ORed with value in range 0x0 - 0xF (refer datasheet, p. 34)
#define SSD1306_SET_MEM_ADDR_MODE                                              \
  0x20 // initiate memory addressing mode setup (refer p. 34-35 in datasheet)
...
#define SSD1306_SET_PAGE_START_ADDR                                            \
  0xB0 // should be ORed with value in range 0x0 - 0x7 (refer datasheet, p. 36)
...
#define SSD1306_SET_COM_PINS_HW_CONFIG                                         \
  0xDA // initiate com pin configuration setup

// 5. Timing & Driving Scheme Setting Command Table
#define SSD1306_SET_DISPLAY_CLOCK_DIVIDE                                       \
  0xD5                                    // initiate clock divide ratio setup
#define SSD1306_SET_PRECHARGE_PERIOD 0xD9 // initiate precharge period setup
#define SSD1306_SET_VCOMH_DESELECT_LEVEL                                       \
  0xDB // initiate Vcomh deselect level setup
 ...
 *
 ***********************************************************************/
typedef int (*SSD1306_i2cWrite)(const unsigned char *, const unsigned char);

typedef struct {
  SSD1306_i2cWrite i2cWrite;
  unsigned char screenBuffer[SSD1306_SCREEN_BUFFER_SIZE];
  unsigned char workBuffer[SSD1306_WORK_BUFFER_SIZE];
  unsigned char curX,
      curY; // current coordinates on the screen (measured from top left corner)
